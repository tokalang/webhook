...
| Area | Status | Notes |
| --- | --- | --- |
| JSON and YAML hooks files | partial | Repeatable `-hooks`/`--hooks` JSON and YAML arrays merge into the same bounded hook model. `id`, `execute-command`, literal `pass-arguments-to-command`, `response-message`, command-output flags, `incoming-payload-content-type`, and value/header or payload-HMAC `trigger-rule.match` work. YAML anchors, aliases, tags, directives, and multi-document streams are rejected by Toka's parser. |
| Template hooks files | partial | `-template` (and `--template`) renders a hooks file before JSON/YAML parsing with a restricted Toka `text/template` subset. Webhook injects upstream-compatible unary `getenv`, `cat`, and `credential` helpers; `cat` removes one trailing newline and `credential` uses `CREDENTIALS_DIRECTORY` when present. Full Go template syntax and built-ins are out of scope. |
| Request references | partial | Command environment/argv support `string`, header, URL/query, JSON payload dotted paths (including top-level arrays under upstream-compatible `root`), URL-encoded and multipart text-form fields, bounded XML dotted element paths, raw body, `request.method`, IPv4 `request.remote-addr` (`address:port`), JSON `entire-payload` and `entire-query`, and JSON `entire-headers`; non-JSON complete payloads, multipart files, XML attributes/entities, and IPv6 remain. |
...
| Child cwd/environment/files | partial | `command-working-directory`, per-child environment (`string`, header, raw body, method), and `pass-file-to-command` work. Files use Toka's 0600 exclusive Linux/macOS temporary-file API, default to the system temporary directory, use `command-working-directory` when configured, and are removed after command completion. |
