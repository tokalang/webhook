...
| HTTP server behavior | partial | One request per connection, serial acceptance, configurable `--urlprefix` (default `hooks`), and repeatable global `--header name=value` response headers (including CORS). |
