...
| Request references | partial | Command environment/argv support `string`, header, URL/query, JSON payload dotted paths, raw body, `request.method`, `entire-payload`, `entire-query`, and JSON `entire-headers`; form/XML payload and remote address remain. |
